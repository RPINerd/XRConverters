...
static void XacExporter_Export_Wrapper(const char* pFilePath, Assimp::IOSystem* pIOHandler, const aiScene* pScene, const Assimp::ExportProperties* /*props*/)
{
    XacExporter::Export(pFilePath, pIOHandler, pScene);
}

Assimp::Exporter::ExportFormatEntry XacExporter::Format ( "xac", "EMotionFX actor exporter", ".xac", XacExporter_Export_Wrapper );
...
void XacExporter::ConvertMorphTargets ( ActorNode* pActorNode, ConversionContext& context )
{
    ...
    foreach ( ActorSubMesh& actorSubMesh, pActorMesh->SubMeshes )
    {
        ...
        // system Assimp doesn't expose morph-target arrays directly; use AssimpCompat to retrieve any registered morph targets
        for ( aiMesh* morph : AssimpCompat::GetMorphTargets(pAiMesh) )
        {
            // find mesh index in scene
            int meshIndex = -1;
            for ( int m = 0; m < context.Scene->mNumMeshes; ++m ) if ( context.Scene->mMeshes[m] == morph ) { meshIndex = m; break; }
            if ( meshIndex >= 0 )
                ConvertMorphTarget ( pActorNode, pAiMesh, context.Scene->mMeshes[meshIndex], vertexOffset, context );
        }
