...
// Older code registered member functions with a 3-arg signature. Newer
// Assimp's Exporter expects fpExportFunc with an extra ExportProperties
// parameter. Provide a thin wrapper with the newer signature that calls
// the original implementation for compatibility.
static void XfmExporter_Export_Wrapper(const char* pFilePath, Assimp::IOSystem* pIOHandler, const aiScene* pScene, const Assimp::ExportProperties* /*props*/)
{
    XfmExporter::Export(pFilePath, pIOHandler, pScene);
}

Assimp::Exporter::ExportFormatEntry XfmExporter::Format ( "xmf", "EgoSoft XuMeshFile exporter", ".xml", XfmExporter_Export_Wrapper );
std::string XfmExporter::GameBaseFolderPath;

void XfmExporter::Export ( const char* pFilePath, Assimp::IOSystem* pIOHandler, const aiScene* pScene )
{
    try
    {
...
void XfmExporter::ConvertPartNode ( Component& component, const std::string& parentPartName, const aiScene* pScene, aiNode* pPartNode )
{
...
void XfmExporter::ConvertCollisionNode ( ComponentPart& part, const aiScene* pScene, aiNode* pCollisionNode )
{
    part.CollisionMesh = ConvertMeshNode ( pScene, pCollisionNode, true );
}

void XfmExporter::ConvertLodNode ( ComponentPart& part, int lodIndex, const aiScene* pScene, aiNode* pLodNode )
{
...
std::shared_ptr<XuMeshFile> XfmExporter::ConvertMeshNode ( const aiScene* pScene, aiNode* pNode, bool isCollisionMesh )
{
...
void XfmExporter::CalculatePartSize ( ComponentPart& part, const aiScene* pScene, aiNode* pCollisionNode )
{
...
void XfmExporter::ExtendVertexDeclaration ( aiMesh* pMesh, std::vector<XmfVertexElement>& declaration )
{
...
void XfmExporter::ApplyVertexDeclaration ( std::vector<XmfVertexElement>& declaration, XmfDataBuffer& buffer )
{
...
int XfmExporter::WriteVertexElement ( aiMesh* pMesh, int vertexIdx, XmfVertexElement& elem, byte* pElemData )
{
