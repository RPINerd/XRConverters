...
ActorFile::ChunkReader::ChunkReader ()
{
    ...
    _handlers[EMOFX_CHUNK_XAC_METADATA].push_back ( &ReadMetaDataV2 );

    _handlers[EMOFX_CHUNK_XAC_NODE_HIERARCHY].push_back ( &ReadNodeHierarchyV1 );

    _handlers[EMOFX_CHUNK_XAC_MATERIAL_TOTALS].push_back ( &ReadMaterialTotalsV1 );

    _handlers[EMOFX_CHUNK_XAC_MATERIAL_DEFINITION].push_back ( nullptr );
    _handlers[EMOFX_CHUNK_XAC_MATERIAL_DEFINITION].push_back ( &ReadMaterialDefinitionV2 );

    _handlers[EMOFX_CHUNK_XAC_MESH].push_back ( &ReadMeshV1 );

    _handlers[EMOFX_CHUNK_XAC_BONE_INFLUENCES].push_back ( nullptr );
    _handlers[EMOFX_CHUNK_XAC_BONE_INFLUENCES].push_back ( nullptr );
    _handlers[EMOFX_CHUNK_XAC_BONE_INFLUENCES].push_back ( &ReadBoneInfluencesV3 );

    _handlers[EMOFX_CHUNK_XAC_MORPH_TARGETS].push_back ( &ReadMorphTargetsV1 );
}
