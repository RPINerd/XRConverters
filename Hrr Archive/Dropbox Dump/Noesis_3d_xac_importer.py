###############
# For XRebirth updated version 27.nov.2013 by TargetLost. 
# The (.xac of XRebirth (3D Studio Max 12) ) 
# seems to have an additonal unknown struct ID4 in the Mesh
# see def parse_mesh(self):
################
    ...
    def parse_mesh(self):
        ...
        for i in range(structs - 1):
            ...
            if struct == 0:
                ...
                #added 27.nov.2013 by TargetLost (.xac of XRebirth (3D Studio Max 12) )
                #Unknown struct with structSize 4 Bytes. 
                #The Mesh seems to have a displacement to other meshes
                #but those 4 bytes doesnt seems to be represent an int or a float vlaue.
                self.inFile.seek(numVerts*structSize, 1)
            else:
                #in case further structs become added with this it wont crash anymore
                self.inFile.seek(numVerts*structSize, 1)
